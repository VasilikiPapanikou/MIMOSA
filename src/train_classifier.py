...
class ClassifierTrainer:
    ...
    def infer_protected_groups(self, X, protected_attrs):
        if not protected_attrs:
            raise ValueError("Adversarial debiasing requires at least one sensitive attribute.")
        if len(protected_attrs) > 1:
            if self.debias:
                raise ValueError("Adversarial debiasing currently supports one sensitive attribute at a time.")
            protected_attrs = self.infer_default_protected_attrs(X) or protected_attrs[:1]

        attr_values = {}
        for attr in protected_attrs:
            if attr not in X.columns:
                raise ValueError(f"Sensitive attribute '{attr}' is not present in the training data.")

            values = sorted(pd.Series(X[attr]).dropna().unique().tolist())
            if len(values) < 2:
                raise ValueError(f"Sensitive attribute '{attr}' has fewer than two values.")
            if len(values) > 2:
                raise ValueError(
                    f"Sensitive attribute '{attr}' has {len(values)} values. "
                    "Please choose a binary sensitive attribute for adversarial debiasing."
                )

            attr_values[attr] = {
                "privileged": values[-1],
                "unprivileged": values[:-1],
            }

        privileged_groups = [
            {attr: spec["privileged"] for attr, spec in attr_values.items()}
        ]

        unprivileged_groups = [
            dict(zip(protected_attrs, values))
            for values in product(*(attr_values[attr]["unprivileged"] for attr in protected_attrs))
        ]

        return privileged_groups, unprivileged_groups

    def infer_default_protected_attrs(self, X):
        candidates = [col for col in X.columns if "Sex" in col or "Race" in col]
        candidates = sorted(candidates, key=lambda col: (0 if "Sex" in col else 1, col))
        binary_candidates = [
            col for col in candidates
            if len(pd.Series(X[col]).dropna().unique()) == 2
        ]
        ...
        if model is not None:
            ...
            if model_type == 'MLP':
                ...
                if adversarial_sensitive_attrs:
                    ...
                    protected_attrs = self.infer_default_protected_attrs(X_train)
                
                privileged_groups, unprivileged_groups = self.infer_protected_groups(
                    X_train, protected_attrs
                )
                ...
                clf.fit(aif_dataset)
                best_params = (
                    f"AdversarialDebiasing(debias={self.debias}, "
                    f"adversary_loss_weight={self.adversary_loss_weight}, seed={self.random_seed})"
                )
