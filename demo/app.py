    ...
    try:
        ...
        metric_definitions = {
            'PR': {
                ...
                'g1_error': f"**Unfair:** Significant disparity in positive rates, disadvantaging the **{focus_group_name}** group.",
                'g2_error': f"**Unfair:** Significant disparity in positive rates, disadvantaging the **{rest_group_name}** group.",
                'success': "**Fair:** The difference is not statistically significant.",
                'higher_is_harmful': False
            },
            'TPR': {
        ...
        for i, metric_key in enumerate(metrics_to_display):
            ...
            with cols[i]:
                ...
                if p_value < 0.05:
                    focus_group_disadvantaged = metric_diff > 0 if defs['higher_is_harmful'] else metric_diff < 0
                    if focus_group_disadvantaged:
...
def display_fairness_delta(
    ...
    for i, metric_key in enumerate(metrics_to_display):
        with cols[i]:
            ...
            if improvement < 0:
                st.warning(f"The intervention increased absolute {metric_key} disparity.")
...
def prepare_custom_dataset(raw_df, target_column, protected_to_remove=None):
    protected_to_remove = protected_to_remove or []

    if raw_df is None or raw_df.empty:
        raise ValueError("The uploaded CSV file is empty.")

    if target_column == "Please select..." or target_column not in raw_df.columns:
        raise ValueError("Please select the target column for the uploaded CSV file.")

    if target_column != "Target" and "Target" in raw_df.columns:
        raise ValueError(
            "The uploaded file already has a column named 'Target'. Please select that column as the target, "
            "or rename one of the columns before uploading."
        )

    unknown_removed = [col for col in protected_to_remove if col not in raw_df.columns]
    if unknown_removed:
        raise ValueError(f"Cannot remove unknown column(s): {', '.join(unknown_removed)}")

    if target_column in protected_to_remove:
        raise ValueError("The target column cannot also be removed as a protected attribute.")

    data_initial = raw_df.copy().dropna().reset_index(drop=True)
    if data_initial.empty:
        raise ValueError("No rows remain after removing missing values from the uploaded CSV file.")

    target_values = data_initial[target_column]
    if target_values.nunique(dropna=True) != 2:
        raise ValueError("Custom datasets currently need a binary target column with exactly two classes.")

    target_encoder = LabelEncoder()
    encoded_target = target_encoder.fit_transform(target_values.astype(str))
    class_names = [str(name) for name in target_encoder.classes_]

    if target_column != "Target":
        data_initial = data_initial.rename(columns={target_column: "Target"})

    feature_names = [
        col
        for col in raw_df.columns
        if col != target_column and col not in protected_to_remove
    ]

    data_initial = data_initial[feature_names + ["Target"]].copy()
    data_df = data_initial.copy()
    data_df["Target"] = encoded_target

    categorical_features = [
        col for col in feature_names
        if (
            pd.api.types.is_object_dtype(data_initial[col])
            or isinstance(data_initial[col].dtype, pd.CategoricalDtype)
            or pd.api.types.is_bool_dtype(data_initial[col])
        )
    ]
    ...
    for feature in categorical_features:
        ...
        data_encoded[feature] = encoder.fit_transform(data_initial[feature].astype(str))
        label_encoders[feature] = encoder
        categorical_names[data_encoded.columns.get_loc(feature)] = encoder.classes_

    continuous_features = [feature for feature in feature_names if feature not in categorical_features]
    for feature in continuous_features:
        data_encoded[feature] = pd.to_numeric(data_encoded[feature], errors="coerce")

    if data_encoded[continuous_features].isna().any().any():
        bad_columns = data_encoded[continuous_features].columns[
            data_encoded[continuous_features].isna().any()
        ].tolist()
        raise ValueError(
            "These non-categorical columns could not be converted to numbers: "
            f"{', '.join(bad_columns)}"
        )

    categorical_indices = [data_encoded.columns.get_loc(feature) for feature in categorical_features]
...
def train_and_predict_model(
    ...
    if model is None and training_model_name != "MLP":
...
def get_cached_explanations(
    ...
    """Generate or load explanations for this exact model/run/subset."""
    explanations_dir = os.path.join(os.path.dirname(__file__), "saved_explanations")
    os.makedirs(explanations_dir, exist_ok=True)

    if isinstance(data_subset, np.ndarray):
        subset_for_hash = pd.DataFrame(data_subset)
    else:
        subset_for_hash = data_subset

    try:
        subset_hash = hashlib.sha256(
            pd.util.hash_pandas_object(subset_for_hash, index=True).values.tobytes()
            + repr(list(subset_for_hash.columns)).encode("utf-8")
        ).hexdigest()[:16]
    except (TypeError, ValueError):
        # Keep persistence safe even for an unusual object-valued subset.
        subset_hash = hashlib.sha256(repr(subset_for_hash).encode("utf-8")).hexdigest()[:16]
    ...
    file_name = os.path.join(
        ...
        "{}_{}_{}_{}_{}_{}_explanations.json".format(
            ...
            safe_filename_part(outcome_group),
            subset_hash,
        ),
    )
    ...
    if not st.session_state.app_started:
        ...
        if st.session_state.current_step == 1:
            ...
            if dataset_name == "ACS Data":
                ...
                if uploaded_file:
                    try:
                        ...
                        if st.session_state.get("custom_file_hash") != file_hash:
                            st.session_state.custom_df = pd.read_csv(io.BytesIO(file_bytes))
                            ...
                            st.session_state.saved_custom_target_col = "Please select..."

                        if 'saved_custom_target_col' not in st.session_state:
                            st.session_state.saved_custom_target_col = "Please select..."

                        try:
                            target_index = st.session_state.custom_cols.index(st.session_state.saved_custom_target_col)
                        except ValueError:
                            target_index = 0

                        custom_target_col = st.sidebar.selectbox(
                            "Select target column",
                            st.session_state.custom_cols,
                            index=target_index,
                            key="widget_custom_target_col",
                            on_change=save_widget_state,
                            args=("saved_custom_target_col", "widget_custom_target_col")
                        )

                        if custom_target_col == "Please select...":
                            st.info("Please select the target column for the uploaded CSV file from the sidebar.")
                            st.stop()

                        st.session_state.custom_target_col = custom_target_col
                        data_initial, data_df, data_encoded, target, class_names, feature_names, categorical_features, categorical_indices, categorical_names, label_encoders = prepare_custom_dataset(
            ...
            DATASET_DESCRIPTIONS = {
                ...
                "COMPAS": "The COMPAS dataset is used to predict whether a defendant does not recidivate within two years.",
            ...
            TARGET_NAME_MAP = {
                ...
                "COMPAS": "Two-Year Non-Recidivism (Survived/Recidivated)",
        ...
        # ==============================================================
            ...
            if st.session_state.saved_explanation_type == "Group":
                st.info(
                    "This feature is pending, please use Individual explanations for now."
                )
                st.stop()
        ...
        # ==============================================================
            ...
            if st.session_state.saved_intervention_type == "Please select...":
                ...
                st.markdown(
                    """
                    **Available methods:**
                    - **Remove protected attribute:** available for all models.
                    - **Adversarial Debiasing:** available only when the base model is **MLP**.
                    """
                )
                st.stop()
            ...
            cols = feature_names
            if intervention_type == "Adversarial Debiasing":
                cols = [
                    col for col in feature_names
                    if col in data_encoded.columns and data_encoded[col].nunique(dropna=True) == 2
                ]
                if not cols:
                    st.warning("Adversarial Debiasing requires a binary sensitive attribute, but none are available for this dataset.")
                    st.stop()

            attribute_label = (
                "Select one binary sensitive attribute for debiasing:"
                if intervention_type == "Adversarial Debiasing"
                else "Select one or more attributes to remove:"
            )
            protected_default = [
                attr for attr in st.session_state.saved_protected_attribs
                if attr in cols
            ]
            ...
            if not protected_attributes:
                if intervention_type == "Adversarial Debiasing":
                    st.info("Please select at least one sensitive attribute for adversarial debiasing.")
                else:
                    st.info("Please select at least one attribute to remove before proceeding with the analysis.")
                st.stop()

            if intervention_type == "Adversarial Debiasing" and len(protected_attributes) > 1:
                st.warning("Adversarial Debiasing currently supports one sensitive attribute at a time. Please select only one attribute.")
                st.stop()
            ...
            if apply_int_btn:
                ...
                with st.spinner(f"Applying {intervention_type}..."):
                    ...
                    if selected_state is not None:       
                        ...
                        custom_target_col = st.session_state.get("custom_target_col")
                        data_initial_intervene, data_df_intervene, data_encoded_intervene, target_intervene, class_names_intervene, feature_names_intervene, categorical_features_intervene, categorical_indices_intervene, categorical_names_intervene, label_encoders_intervene = prepare_custom_dataset(
                            custom_df,
                            custom_target_col,
                            protected_to_remove=cols_to_drop_on_load
                        )
                    else:          
                        data_initial_intervene, data_df_intervene, data_encoded_intervene, target_intervene, class_names_intervene, feature_names_intervene, categorical_features_intervene, categorical_indices_intervene, categorical_names_intervene, label_encoders_intervene = load_dataset(
            ...
            if st.session_state.intervention_trained and 'intervention_results' in st.session_state:
                ...
                if st.session_state.int_fairness_calculated:
                    ...
                    st.subheader(f"Group Fairness metrics after {intervention_type}.")
                ...
                if st.session_state.saved_int_exp_type == "Group":
                    st.info(
                        "This feature is pending, please use Individual explanations for now."
                    )
                    st.stop()
